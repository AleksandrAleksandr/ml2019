...
def test_define_network(environment, net):
    ...
    assert isinstance(get_action(s, network=network), int), \
        'get_action(s) must return int, not %s. try int(action)' % (type(get_action(s, network)))

    print('Test #1: define_network() & get_action() functions: OK!')


def test_eps_greedy_strategy(network):
    # Test epsilon-greedy exploration
    for eps in [0., 0.1, 0.5, 1.0]:
        state_frequencies = np.bincount([get_action(s, network=network, epsilon=eps) for i in range(10000)], minlength=n_actions)
...
if __name__ == '__main__':
    ...
    # test_eps_greedy_strategy(network)
