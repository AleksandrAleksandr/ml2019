...
class QLearningAgent:
    ...
    def get_state_value(self, state):
        ...
        q_value = max([self.get_q_value(state,a) for a in possible_actions])
        return q_value
    ...
    def get_best_policy_action(self, state):
        ...
        best_action = {a:self.get_q_value(state,a)for a in possible_actions}
        max_q,max_ind =0.,0
    ...
    def get_action(self, state):
        ...
        if r == 0:
          a = np.random.choice(len(possible_actions))
        else:
          a = self.get_best_policy_action(state)
        return a
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        q_est = reward + self.gamma * self.get_state_value(state)
        value = self.alpha * q_est + (1-self.alpha) * self.get_q_value(state,action)
        self.set_q_value(state,action,value)
